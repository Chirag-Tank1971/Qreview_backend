  ...
  try {
    ...
    const reportData = reviews.map((r) => {
      ...
      return {
        ...
        cycleCode: r.cycleCode || emp?.cycleCode || 'N/A',
        cycleName: emp?.cycleName || 'Unassigned',
  ...
  try {
    ...
    const reportData = eligibleEmployees.map((emp) => {
      ...
      return {
        ...
        departmentName: emp.departmentName || 'General',
        designationName: emp.designationName || 'Specialist',
        cycleCode: cycle?.code || emp.cycleCode || 'N/A',
        cycleName: cycle?.name || emp.cycleName || 'Unassigned',
