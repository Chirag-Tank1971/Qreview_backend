 ...
 * Executive bird's-eye view for Board / CXO / Management
 */
dashboardRouter.get(
  '/dashboard/management',
  requireRoles('MANAGEMENT', 'SUPER_ADMIN', 'HR'),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const empCol = getDbCollection('employees');
      const deptCol = getDbCollection('departments');
      ...
      const currentReviews: EmployeeReview[] = currentPeriod
        ? await (await reviewCol.find({ reviewPeriodId: currentPeriod.id })).toArray()
        : [];

      const scoredReviews = currentReviews.filter((r) => (r.finalScore || 0) > 0);
      const orgAverageScore =
        scoredReviews.length > 0
          ? Number((scoredReviews.reduce((a, b) => a + (b.finalScore || 0), 0) / scoredReviews.length).toFixed(2))
          : 0;
      ...
      const totalPayrollPre = appraisals.reduce((a, b) => a + (b.currentCtc || 0), 0);
      const totalPayrollPost = appraisals.reduce((a, b) => a + (b.revisedCtc || b.currentCtc || 0), 0);
      const totalBudgetSpent = totalPayrollPost - totalPayrollPre;

      const highPerformers = appraisals.filter((a) => a.averageQuarterlyScore >= 4.2);
      const flightRisks = highPerformers.filter(
        (a) => (a.approvedIncrementPercentage || a.proposedIncrementPercentage || 0) <= 10
      );
      ...
      res.json({
        ...
        reviewCompletionRate:
          currentReviews.length > 0
            ? Math.round(
                (currentReviews.filter((r) => r.isClosed || r.status === 'CLOSED').length / currentReviews.length) *
                  100
              )
            : 0,
        payrollMetrics: {
          currentPayroll: totalPayrollPre,
          revisedPayroll: totalPayrollPost,
          budgetImpact: totalBudgetSpent,
          averageIncrement:
            appraisals.length > 0
              ? Number(
                  (
                    appraisals.reduce(
                      (a, b) => a + (b.approvedIncrementPercentage || b.proposedIncrementPercentage || 0),
                      0
                    ) / appraisals.length
                  ).toFixed(1)
                )
      ...
      res.status(500).json({ error: 'Failed to load management dashboard data.' });
    }
  }
);
