...
// Helper to check user from Authorization header
...
/**
 * GET /api/appraisals
 * List annual appraisals with filtering & RBAC
 */
appraisalRouter.get('/appraisals', async (req: Request, res: Response) => {
  try {
    ...
    if (user) {
      ...
      } else if (user.role === 'MANAGER' && user.employeeId && (onlyMine === 'true' || user.role === 'MANAGER')) {
        appraisals = appraisals.filter(
          (a) => a.managerId === user.employeeId || a.employeeId === user.employeeId
        );
      } else if (user.role === 'HOD' && user.employeeId && onlyMine === 'true') {
        appraisals = appraisals.filter(
          (a) => a.hodId === user.employeeId || a.managerId === user.employeeId || a.employeeId === user.employeeId
        );
      }
    }
    ...
    if (departmentId && departmentId !== 'ALL') {
      appraisals = appraisals.filter((a) => a.departmentId === departmentId || a.departmentName?.toLowerCase().includes((departmentId as string).toLowerCase()));
    }
    if (managerId) {
    ...
    // Sort: most recent appraisal year/month, then by pending status priority
    appraisals.sort((a, b) => {
 ...
 * Cohort metrics, budget pool, and calibration distribution
 */
appraisalRouter.get('/appraisals/stats', async (req: Request, res: Response) => {
  try {
 ...
 */
appraisalRouter.get('/appraisals/analytics/executive', async (req: Request, res: Response) => {
  try {
    ...
    allDepartments.forEach((dept) => {
      ...
      // Check skew alerts (e.g. forced distribution target: Top 10%, Exceeds 25%, Meets 45%, Needs Imp 20%)
    ...
    allAppraisals.forEach((a) => {
      ...
      const isUndercompensated = increment < 14 && isTopPerformer;
 ...
 * Retrieve single appraisal details with full 4-quarter review snapshot
 */
appraisalRouter.get('/appraisals/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const appraisalsCol = getDbCollection('appraisals');
 ...
 * Batch roll up 4-quarter reviews and initiate annual appraisals for an 8-Cycle cohort
 */
appraisalRouter.post('/appraisals/initiate-cycle', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    ...
    const audit: AuditLog = {
      ...
      module: 'ANNUAL_APPRAISAL',
      action: 'BATCH_INITIATE_COHORT',
      recordId: cycle.id,
      newValue: `Initiated ${createdCount + updatedCount} appraisals for ${cycle.name} (${appraisalYear})`,
      details: `Generated rolling 4-quarter performance rollups and standard increment recommendations.`,
      createdAt: new Date().toISOString(),
    };
    await auditLogsCol.insertOne(audit);
 ...
 */
appraisalRouter.put('/appraisals/:id/manager-recommend', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    const { id } = req.params;
    const {
    ...
    await auditLogsCol.insertOne({
      id: `audit_${Date.now()}`,
      userId: user?.id || 'usr_manager',
      userName: user?.name || 'Manager',
      userRole: user?.role || 'MANAGER',
      module: 'APPRAISAL_CALIBRATION',
      action: 'MANAGER_RECOMMENDATION_SUBMITTED',
      recordId: id,
      newValue: `Suggested ${incPercent}% increment, Promotion: ${promotionRecommended ? 'YES' : 'NO'}`,
 ...
 */
appraisalRouter.put('/appraisals/:id/hod-calibrate', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    const { id } = req.params;
    ...
    await auditLogsCol.insertOne({
      id: `audit_${Date.now()}`,
      userId: user?.id || 'usr_hod',
      userName: user?.name || 'HOD',
      userRole: user?.role || 'HOD',
      module: 'APPRAISAL_CALIBRATION',
      action: 'HOD_CALIBRATION_COMPLETED',
      recordId: id,
      newValue: `Calibrated to ${finalInc}% increment`,
      details: notes,
      createdAt: new Date().toISOString(),
    });
 ...
 */
appraisalRouter.put('/appraisals/:id/hr-approve', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    const { id } = req.params;
    ...
    await auditLogsCol.insertOne({
      id: `audit_${Date.now()}`,
      userId: user?.id || 'usr_hr',
      userName: user?.name || 'HR Manager',
      userRole: user?.role || 'HR',
      module: 'APPRAISAL_CALIBRATION',
      action: 'HR_FINAL_APPROVAL',
      recordId: id,
      newValue: `Final approved increment: ${approvedInc}%, Revised CTC: ${revisedCtc}`,
      details: notes,
      createdAt: new Date().toISOString(),
    });
 ...
 */
appraisalRouter.put('/appraisals/:id/lock', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    const { id } = req.params;
    ...
    await auditLogsCol.insertOne({
      id: `audit_${Date.now()}`,
      userId: user?.id || 'usr_super_admin',
      userName: user?.name || 'Administrator',
      userRole: user?.role || 'SUPER_ADMIN',
      module: 'ANNUAL_APPRAISAL',
      action: 'APPRAISAL_LOCKED',
      recordId: id,
      newValue: `Locked appraisal. Updated Employee CTC to ${appraisal.currency}${appraisal.revisedCtc.toLocaleString()}`,
      createdAt: lockedAt,
    });
 ...
 * Generates formatted Appraisal & Promotion letter data
 */
appraisalRouter.get('/appraisals/:id/letter', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const appraisalsCol = getDbCollection('appraisals');
    const appraisal: Appraisal | null = await appraisalsCol.findOne({ id });
 ...
 */
appraisalRouter.put('/appraisals/:id/acknowledge', async (req: Request, res: Response) => {
  try {
    const user = await authenticateUser(req);
    const { id } = req.params;
    ...
    const ackRecord = {
      acknowledged: true,
      acknowledgedAt,
      acknowledgedBy: user?.id || 'usr_employee',
    ...
    await auditLogsCol.insertOne({
      id: `audit_${Date.now()}`,
      userId: user?.id || 'usr_employee',
      userName: user?.name || appraisal.employeeName,
      userRole: user?.role || 'EMPLOYEE',
      module: 'ANNUAL_APPRAISAL',
      action: 'APPRAISAL_ACKNOWLEDGED',
      recordId: id,
      newValue: `Digitally acknowledged by ${appraisal.employeeName}`,
      details: comments || 'Digital acceptance of appraisal letter and compensation revision terms.',
